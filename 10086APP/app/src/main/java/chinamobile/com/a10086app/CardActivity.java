...
/**
	...
	*/
/*public CardActivity(Context c, int w, int h) {
		context=c;
		// 用于PopupWindow的View
		contentView=LayoutInflater.from(c).inflate(R.layout.subscribe_activity, null, false);
		initView();
		initData();
...
public class CardActivity extends Activity implements OnItemClickListener {
	...
	private void initData() {
		...
		userChannelList = defaultUserChannels;
		otherChannelList = defaultOtherChannels;
		userAdapter = new DragAdapter(this, cards);
		userGridView.setAdapter(userAdapter);
		//otherAdapter = new OtherAdapter(this, otherChannelList);
		//otherGridView.setAdapter(this.otherAdapter);
		//设置GRIDVIEW的ITEM的点击监听
...
/*		cards.add(new ProgressCard(100,80,"语音","剩余流量/GB"));
		cards.add(new ProgressCard(100,70,"语音","剩余流量/GB"));*/
		...
		switch (parent.getId()) {
			case R.id.userGridView:
				...
				if (position != 0 && position != 1) {
					final ImageView moveImageView = getView(view);
					if (moveImageView != null) {
						TextView newTextView = (TextView) view.findViewById(R.id.card_text_title);
						final int[] startLocation = new int[2];
						newTextView.getLocationInWindow(startLocation);
						final TextCard channel = (TextCard) (((DragAdapter) parent.getAdapter()).getItem(position));//获取点击的频道内容
						//otherAdapter.setVisible(false);
						//添加到最后一个
						//otherAdapter.addItem(channel);
						new Handler().postDelayed(new Runnable() {
			...
			case R.id.otherGridView:
				...
				if (moveImageView != null){
					...
					final int[] startLocation = new int[2];
					newTextView.getLocationInWindow(startLocation);
					final TextCard channel = ((OtherAdapter) parent.getAdapter()).getItem(position);
					userAdapter.setVisible(false);
					//添加到最后一个
					//userAdapter.addItem(channel);
					new Handler().postDelayed(new Runnable() {
	 ...
	 * @param clickGridView
	 */
	private void MoveAnim(View moveView, int[] startLocation,int[] endLocation, final TextCard moveChannel,
						  final GridView clickGridView) {
		int[] initLocation = new int[2];
		//获取传递过来的VIEW的坐标
		moveView.getLocationInWindow(initLocation);
